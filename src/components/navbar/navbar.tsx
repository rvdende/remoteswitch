...
export function Navbar(props: {
    ...
    >
      {mounted ? (
        <>
          {currentTheme === "dark" ? (
            <SunIcon
              className=":hover:text-zinc-400 h-6 w-6 transition-all"
              aria-hidden="true"
            />
          ) : (
            <SunIcon
              className=":hover:text-yellow-200 h-6 w-6 text-orange-400 transition-all"
              aria-hidden="true"
            />
          )}
        </>
  ...
  return (
    <Disclosure as="nav" className="bg-white transition dark:bg-zinc-900">
      {({ open }) => (
        <>
          <Container>
            <div className="relative flex h-16 items-center justify-between">
                  ...
                  <div className="w-full max-w-lg lg:max-w-xs">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        name="search"
                        className={clsx(
                          `block w-full rounded-md border border-transparent py-2 pl-10 pr-3 leading-5 transition-all focus:outline-none sm:text-sm`,
                          `bg-zinc-100 text-zinc-700 placeholder-zinc-400 outline-none hover:border-zinc-200 hover:bg-zinc-200 focus:border-zinc-300 focus:bg-white focus:text-zinc-500 focus:placeholder-transparent focus:ring-zinc-300`,
                          `hover:bg-zinc-600 dark:bg-zinc-700 dark:text-zinc-300 dark:placeholder-zinc-400 dark:hover:border-zinc-500 dark:focus:border-zinc-700 dark:focus:bg-zinc-700 dark:focus:text-zinc-300 dark:focus:placeholder-transparent dark:focus:ring-zinc-700`
                        )}
                        placeholder="Search"
                        type="search"
                      />
              ...
              <div className="flex lg:hidden">
                <div className="mr-2 mt-1 flex flex-row space-x-2">
                  <div className="">
                    <DarkMode />
                  </div>
              ...
              <div className="hidden lg:ml-4 lg:block">
                <div className="flex items-center">
                  ...
                  {session.status === "unauthenticated" ? (
                    <>
                      <Link href="/signin">
                        <button className="ml-4 text-sm opacity-70">
                          Sign In
                        </button>
                      </Link>
            ...
            <div className="space-y-1 px-2 pt-2 pb-3">
              ...
              {props.nav?.map((m) => (
                <Disclosure.Button
                  ...
                  className={clsx(
                    m.active
                      ? "block rounded-md bg-zinc-900 px-3 py-2 text-base font-medium text-white"
            ...
            <div className="bg-zinc-100 pt-0 pb-3 shadow-xl dark:bg-zinc-700">
              ...
              {session.status === "unauthenticated" && (
                <div className="mt-3 px-2">
                  ...
                  <Chiplet title="beta" description="Sign Up" href="/signup" />
                </div>
              )}
            </div>
          </Disclosure.Panel>
...
export const AvatarCircle = () => {
  ...
  return (
    <div className="font-sm h-8 rounded-full bg-zinc-200 bg-opacity-90 px-3 pt-1.5 font-semibold text-zinc-500 dark:bg-zinc-700 dark:text-zinc-300">
...
export const NavBarAvatarMenu = () => {
  return (
    <Menu as="div" className="relative ml-2 flex-shrink-0">
      ...
      >
        <Menu.Items className="absolute right-0 z-10 mt-2 w-48 origin-top-right overflow-hidden rounded-md bg-white shadow-xl ring-1 ring-black ring-opacity-5 focus:outline-none dark:bg-zinc-700">
          {NavBarProfileMenu.map((nav) => (
              ...
              {({ active }) => (
                <Link
                  ...
                  className={clsx(
                    ...
                    active
                      ? "bg-zinc-100 text-zinc-500 dark:bg-zinc-600 dark:text-emerald-500"
                      : "text-zinc-400"
                  )}
                >
                  {nav.name}
